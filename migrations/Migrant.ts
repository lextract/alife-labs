...
export class Migrant {
    ...
    private targetPlot: LandPlot;
    private currentPlot: LandPlot;
    private eat: boolean = false;
    constructor(
        public x: number,
        public y: number,
        private world: LandMigrations
    ) {
    ...
    perceive() {
        ...
        if (!td) this.targetDirection = valids[0];
    }
    ...
    private findResources(dir: Direction): LandPlot {
        let plot: LandPlot;
        if (dir == Direction.Up) {
            ...
            for (let j = this.y - 1; j >= yLimit; j--) {
                ...
                if (plot.levelResources > 0) break;
            }
        }
        ...
        if (plot.levelResources <= 0) plot = undefined;
        return plot;
    }
    private validDirections(randomly?: boolean): Direction[] {
        let up = this.y - 1 >= 0;
        if (up) {
        ...
        if (!randomly) {
            if (up) validDirections.push(Direction.Up);
            if (right) validDirections.push(Direction.Right);
            if (down) validDirections.push(Direction.Down);
            if (left) validDirections.push(Direction.Left);
        ...
        while (counterDirections < 4) {
            ...
            if (!processedDirs[dir]) {
                ...
                counterDirections++;
                switch (dir) {
                    case 0: if (up) validDirections.push(Direction.Up); break;
                    case 1: if (right) validDirections.push(Direction.Right); break;
                    case 2: if (down) validDirections.push(Direction.Down); break;
                    case 3: if (left) validDirections.push(Direction.Left); break;
                }
            }
        }
        return validDirections;
    ...
    draw(ctx: CanvasRenderingContext2D) {
        ...
        ctx.beginPath();
        ctx.arc(
            x + this.world.land.migrantMeasures.xCenter,
            y + this.world.land.migrantMeasures.yCenter,
            this.world.land.migrantMeasures.radius,
            this.world.land.migrantMeasures.startAngle,
            this.world.land.migrantMeasures.endAngle
        );
        ctx.lineTo(
            x + this.world.land.migrantMeasures.xCenter - this.world.land.migrantMeasures.xGapMouth,
            y + this.world.land.migrantMeasures.yCenter
        );
        ctx.fill();
        ...
        ctx.beginPath();
        ctx.arc(
            x + this.world.land.migrantMeasures.xEyeCenter,
            y + this.world.land.migrantMeasures.yEyeCenter,
            this.world.land.migrantMeasures.eyeRaduis,
            0,
            -Math.PI,
            false
        )
